...
# from .bimamba_ssm.modules.mamba_simple2 import Mamba
from .bimamba_ssm.modules.def_mamba_simple import Mamba
...
from .serialization_ import Point as Point
...
class Mamba3DBlock(nn.Module):
    def __init__(self,
        ...
        self.mixer = Mamba(d_model=dim, mamba_type=bimamba_type,enable_dp=True,enable_dt=True)

        # self.ema = EMA(channels=384, factor=32)
    ...
    def forward(self, center, x, order):
        ...
        # x1 = rearrange(x, "b d l -> b l d")
        # x2=self.ema(x1)
        # x3 = rearrange(x2, "b l d -> b d l")
        # # x + norm(x)->mamba(x)->dropout
        # x = x + self.drop_path(self.mamba_shuffle(x))
        x1, new_coord, new_order = self.mixer(self.norm2(x),coords=center,base_idx=order)
        x = x + self.drop_path(x1)

        return x, new_coord, new_order
...
class Mamba3DEncoder(nn.Module):
    ...
    def forward(self, center, x, pos, order):
        '''
        INPUT:
        ...
        for _, block in enumerate(self.blocks):
            x, new_coord, new_order = block(center, x + pos, order)
            # center = new_coord
            # order = new_order
        return x, new_coord, new_order
...
class DM3D(nn.Module):
    ...
    def forward(self, pts):
        ...
        center, order, inverse_order, group_input_tokens, pos = serialization_func(center, group_input_tokens, pos, "hilbert")

        x = torch.cat((cls_tokens, group_input_tokens), dim=1)
        pos = torch.cat((cls_pos, pos), dim=1)

        # transformer
        x, new_coord, new_order = self.blocks(center, x, pos, order)  # enter transformer blocks
        x = self.norm(x)
