...
def parse_args():
    '''PARAMETERS'''
    parser = argparse.ArgumentParser('DM3D')
    parser.add_argument('--model', type=str, default='DM3D', help='model name')
    ...
    parser.add_argument('--log_dir', type=str, default='output/part_seg/1014_1340/', help='experiment root')
...
def main(args):
    ...
    file_handler = logging.FileHandler(('%s/test-' + str(time_now) + '.txt') % (experiment_dir))
    ...
    #     classifier.load_model_from_ckpt(str(experiment_dir) + '/best_model.pth')
    ...
    model_state_dict = checkpoint['model_state_dict']  # 提取模型参数部分
    classifier.load_state_dict(model_state_dict, strict=False)  # 允许部分加载
