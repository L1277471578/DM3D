...
model_path = args1.model_path + '/model.txt'
...
def run_net(args, config, train_writer=None, val_writer=None):
    ...
    for i in range(test_total_num):
        ...
        dummy_input = torch.randn(1, 2048, 3)
        calflops, macs, calparams = calculate_flops(base_model.cuda(), input_shape=(1, 2048, 3))
        flops, params = profile(base_model, (dummy_input.cuda(),))
        ...
        with open(model_path, "a") as f:
            f.write('dataset：   ' + str(dataset) + '\n' +
                    'cls_num:    ' + str(config.model.cls_dim) + '\n\n' +
                    'MACs: ' + macs + '  FLOPs：' + calflops + '   Params：' + calparams + '\n' +
                    'FLOPs: ' + str(int(flops)) + ' = ' + str('%.4fM ' % (flops / 1000000.0)) +
                    ' = ' + str('%.4fG' % (flops / 1000000000.0)) + '   ' + '\n' +
                    'params: ' + str(int(total_parameters)) + ' = ' + str('%.4fM ' % (total_parameters / 1000000.0)) +
                    ' = ' + str('%.4fG' % (total_parameters / 1000000000.0)) + '   ' +
                    '\n' +
                    str(summary(base_model, input_size=(1, 2048, 3), depth=12, device='cuda')) + '\n\n\n' +
                    str(base_model))
        ...
        for epoch in tqdm(range(start_epoch, config.max_epoch + 1)):
            ...
            if epoch % args.val_freq == 0 and epoch != 0:
                ...
                if better:
                    ...
                    builder.save_checkpoint(base_model, optimizer, epoch, metrics, best_metrics_vote, 'ckpt-best',args, logger=logger)
                    print_log(
                        "--------------------------------------------------------------------------------------------",
                        logger=logger)
                # if args.vote:
