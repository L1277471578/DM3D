...
from .runner_tsne import visualize_confusion_matrix
import numpy as np
...
def run_net(args, config, train_writer, val_writer):
    ...
    if config.dataset.train._base_.NAME == 'ModelNet' and config.dataset.test._base_.NUM_CATEGORY != config.model.cls_dim:
        sys.exit("config cls_num != dataset cls_num")

    # build model
    base_model = builder.model_builder(config.model)
    num_trainable_params = sum(p.numel() for p in base_model.parameters() if p.requires_grad)
    print_log(f"Total Number of trainable parameters: {num_trainable_params}", logger=logger)
    ...
    # calflops, macs, calparams = calculate_flops(base_model.cuda(), input_shape=(1, 2048, 3))
    flops, params = profile(base_model.cuda(), (dummy_input.cuda(),))
    print('\n******************FPLOs:',str(flops/1e9),"G FLOPs")
    ...
    for epoch in tqdm(tqdm(range(start_epoch, config.max_epoch + 1)), mininterval=1, maxinterval=2):
        if args.distributed:
        ...
        for idx, (taxonomy_ids, model_ids, data) in enumerate(train_dataloader):
            ...
            if idx % 20 == 0:
                print('[Epoch %d/%d][Batch %d/%d] BatchTime = %.3f (s) DataTime = %.3f (s) Loss+Acc = %s lr = %.6f' %
...
def fps_sample_point_cloud(points, n_samples=1024):
    """使用FPS采样点云到指定点数"""
...
def test_net(args, config):
    ...
    # test_vote(base_model, test_dataloader, args, config, logger=logger, times=10)
...
def test(base_model, test_dataloader, args, config, logger=None):
    ...
    with torch.no_grad():
        ...
        #
        # acc = 0.
        # for time in tqdm(range(1, 300),maxinterval=0.5,postfix='test vote'): # 300
    ...
    cm_percent = visualize_confusion_matrix(test_pred, test_label)
    confusion_matrix2(config, cm_percent)
